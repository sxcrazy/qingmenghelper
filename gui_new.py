...
async def get_match_history(connection, puuid, game_name, label=""):
    try:
        ...
        if resp.status != 200:
            gui_print(f"[提示] 无法获取战绩 (Status: {resp.status})")
            return
            
        data = await resp.json()
        matches = data.get('games', {}).get('games', [])
        if not matches:
            gui_print(f"[提示] 该玩家暂无对局记录。")
            return
        ...
        for match in matches[:10]:
            ...
            gui_print(f" - 英雄: {champion_name:15} | 结果: {result} | KDA: {kills}/{deaths}/{assists} | 时间: {gameCreationDate}")
            
    except Exception as e:
        gui_print(f"[错误] 获取战绩时发生异常: {e}")
...
async def get_player_history(connection, player_puuid, player_name, label="玩家", champion_name=""):
    ...
    try:
        ...
        gui_print(f"[提示] 查询 {label} {player_name} 时发生错误：{e}")
...
async def monitor_one_game(connection):
    ...
    try:
        ...
        if resp.status == 200:
            ...
            queue_names = {440: "灵活排位", 420: "单排/双排", 450: "极地大乱斗", 2400: "海克斯大乱斗", 430: "匹配模式", 3140: "训练营"}
...
class MainWindow(QMainWindow):
    ...
    def on_loop_ready(self):
        ...
        gui_print("[系统] 后台通信已准备就绪，您可以开始搜索了！")
    ...
    def start_search(self):
        ...
        if not full_text:
            self.append_log("[系统] 输入不能为空")
            return
        if '#' not in full_text:
            self.append_log("[系统] 输入格式错误，请使用 名字#编号 的格式")
            return

        game_name, tag_line = full_text.split('#', 1)

        if not self.connection:
            self.append_log("[系统] 尚未连接到客户端，无法搜索")
            return

        global monitor_loop
        if monitor_loop is None or monitor_loop.is_closed():
            self.append_log("[系统] 后台通信循环未准备好，请稍后再试！")
            return
        ...
        try:
            ...
            self.append_log(f"[错误] 提交搜索任务失败：{e}")
            self.search_btn.setEnabled(True)
            self.result_label.setText("")
