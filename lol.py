...
async def get_player_history(connection, player_puuid, player_name, label="玩家", champion_name=""):
    ...
    # KDA计算
    if total_deaths == 0:
        avg_kda = (total_kills + total_assists)  # 死亡0次
    else:
        avg_kda = (total_kills + total_assists) / total_deaths #正常情况
    #分数计算
    ...
    print(f"\n[战绩] {label} {display_name} | 胜率 {win_rate:.1f}% | KDA {avg_kda:.2f} | 综合评分 {score:.1f} | 评价： {tag}")
